...
if (!class_exists('ESIG_PDF_Admin')) :
    ...
    class ESIG_PDF_Admin {
        ...
        public function display_pdf_button($result, $docId) {
            ...
            $pdfurl = add_query_arg(array('esigtodo' => 'esigpdf', 'did' => $csum), get_permalink(WP_E_Sig()->setting->get_generic("default_display_page")));
            // 
            if (wp_is_mobile()) {
...
<div class='document-sign-page'>";
        ...
        public function get_signature_image_url($user_id, $document_checksum) {
            WP_E_Sig()->signature->esign_set_json($user_id, $document_checksum);
            $nonce = wp_create_nonce($user_id . $document_checksum);
            $image_url = (ESIGN_DIRECTORY_URI . 'lib/sigtoimage.php?uid=' . $user_id . '&doc_id=' . $document_checksum . '&esig_verify=' . $nonce);
        ...
        public function esign_set_json($user_id, $csum_id, $owner_id = false) {
            ...
            $file_name = plugin_dir_path(__FILE__) . 'assets/' . $user_id . '-' . $csum_id . '.txt';
