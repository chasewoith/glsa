...
if (!class_exists('ESIG_SIF')) :
    ...
    class ESIG_SIF {
        ...
        public function sif_upload_file() {
            ...
            $up_dir = $upload_dir . "/esign/" . $file_name;

            $move_file = move_uploaded_file($_FILES['file']['tmp_name'], $up_dir);

            if ($move_file) {
                $data[0] = "success";
                $data[1] = $upload_dir_list['baseurl'] . "/esign/" . $file_name;
                $data[2] = $file_name;
                echo json_encode($data);
                die();
            } else {
        ...
        private function file_to_html($value = '', $name, $signed = false, $verifysigner = 'undefined', $is_required = false, $label = false, $extensions = false, $filesize = false) {
            ...
            if ($signed) {
                ...
                return '<div class="sif-file-container"><label><a href="' . $value . '" target="_blank">' . $basename . ' -Download File</a></label></div>';
    ...
    j.ajax({
                ...
                success: function(data){
					...
					if(data[0] == "success")
					{
						...
						j("#' . $name . '").val("");
						j("#file-' . $name . '").hide();
						var up_data ="<span id=\"pallate-' . $name . '\"><input type=\"hidden\" name=\"' . $name . '\" value=\""+ data[1] +"\"><a href=\""+ data[1] +"\" target=\"_blank\">"+ data[2] + "-Download File</a></span><span class=\"glyphicon glyphicon-trash sif-icon-size\" title=\"Remove this file\" id=\"' . $name . '1\"></span>";
